...
@Component class DemoAgentRuntime implements AgentRuntime {/**
                                                            * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
                                                            */
public AgentResult run(AgentRequest request){return new AgentResult("local-audit-demo","已匹配控制点、抽取证据并形成底稿草案，审计结论等待签署。",List.of(new AgentStep("控制匹配","COMPLETED","匹配 9 个关键控制"),new AgentStep("证据测试","COMPLETED","完成 24 个样本核验"),new AgentStep("结论签署","PENDING","等待项目经理复核")),Map.of("controls",9,"samples",24,"objectiveLength",request.objective().length()));}}
