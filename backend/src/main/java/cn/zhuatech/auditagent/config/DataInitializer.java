...
@Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
 ...
 reviews.saveAll(List.of(new ReviewRecord("WP-260801-118",a,"控制有效性",24,2,ReviewRecord.Result.PENDING,"陆衡"),new ReviewRecord("WP-260801-096",b,"证据完整性",20,0,ReviewRecord.Result.PASSED,"简宁"),new ReviewRecord("WP-260731-137",c,"样本偏差",22,4,ReviewRecord.Result.FAILED,"宋惟")));
 String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"简宁",UserAccount.Role.DOMAIN_USER,"AUD-PROCURE"),new UserAccount("planner",demo,"陆衡",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"宋惟",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}}
